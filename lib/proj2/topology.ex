defmodule Topology do
  ...
  def initialize_honeycomb_random_table(n) do
    table = :ets.new(:honeycombrandom, [:named_table])
    root=:math.pow(n,0.5) |>round
    square = :math.pow(root,2) |> round

    map =
      Enum.reduce(1..n, %{}, fn node_id, acc ->
        Map.put(acc, node_id, find_honeycomb_random_neighbour(node_id, root, square))
      end)

    :ets.insert(table, {"data", map})
  end

  def find_honeycomb_random_neighbour(node_id, row_length, total) do
    # top
    list =
      cond do
        # Deals the 1st column
    ...
    valid_list = Enum.to_list(1..total)
    # Reject the neighbors which are invalid nodes
    list = Enum.filter(list, fn el -> Enum.member?(valid_list,el) end)
    list ++ [:rand.uniform(total)]
  end

  def lookup_honeycomb_random_neighbour(node_id) do
    [{_, map}] = :ets.lookup(:honeycombrandom, "data")
    map[node_id]
  end
